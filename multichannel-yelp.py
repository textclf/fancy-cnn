...
if __name__ == '__main__':
    ...
    train['text4yelp'] = np.load('Yelp_train_glove_X.npy')
    train['text4global'] = np.load('Yelp_train_global_glove_X.npy')
    ...
    weights = 1.0 * ((train['text4yelp'] > 0) | (train['text4global'] > 0))
    ...
    # -- testing data
    test['text4yelp'] = np.load('Yelp_test_glove_X.npy')
    test['text4yelp'] = test['text4yelp'].reshape(test['text4yelp'].shape[0], -1)

    test['text4global'] = np.load('Yelp_test_global_glove_X.npy')
...
#            sample_weight = {'prediction' : weights}, 
    ...
    yhat = graph.predict({'yelp_input' : test['text4yelp'], 'glove_input' : test['text4global'], }, verbose=True, batch_size=50)
